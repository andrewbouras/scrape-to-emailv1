...
SEARCH_QUERY = {
    ...
    "main": (
        ...
        'AREA[OverallStatus]"RECRUITING" AND '
        ...
        if raw_studies and len(contact_data) > 0:
            ...
            for study_info in contact_data:
                for contact in study_info['contacts']:
                    ...
                    contact_key = get_unique_contact_key(contact)
                    if contact_key not in unique_contacts:
                        unique_contacts[contact_key] = contact
                    else:
                        # Update with better contact info if available
                        best_contact = get_best_contact_info([unique_contacts[contact_key], contact], contact_key)
                        unique_contacts[contact_key] = best_contact
            
            # Generate emails only for unique contacts
